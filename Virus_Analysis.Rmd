...
curve(2.094528e+05/(1+832.12*exp(x*-0.108)), col = "blue", add = TRUE)
...
my_counties %>% filter(Date == "2020-05-05") %>% group_by(Province_State) %>% summarize(state_total = sum(Confirmed)) %>% top_n(state_total,n=20) %>% arrange(desc(state_total))
my_counties %>% filter(Date == "2020-05-05") %>% group_by(Province_State) %>% summarize(state_total = sum(Confirmed)) %>% top_n(state_total,n=-20) %>% arrange(desc(state_total))
