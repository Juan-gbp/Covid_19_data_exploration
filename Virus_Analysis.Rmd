...
curve(1.772915e+05/(1+2716.7*exp(x*-0.136)), col = "blue", add = TRUE)
...
my_counties %>% filter(Province_State == "Pennsylvania" & Date == "2020-04-19")
...
my_counties %>% filter(Date == "2020-04-19") %>% group_by(Province_State) %>% summarize(state_total = sum(Confirmed)) %>% top_n(state_total,n=20) %>% arrange(desc(state_total))
my_counties %>% filter(Date == "2020-04-19") %>% group_by(Province_State) %>% summarize(state_total = sum(Confirmed)) %>% top_n(state_total,n=-20) %>% arrange(desc(state_total))
...
#Junk
```{r}
ourfinaldata %>% filter(Country_Region == "Brazil") %>% group_by(Date) %>%  ggplot() + geom_line(aes(x = Date, y = Confirmed,group =1), linetype = "dashed")  + geom_line(aes(x = Date, y = Deaths,group =1), color = "red")+ ggtitle(" Cases and Deaths")
```
