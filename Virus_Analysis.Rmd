...
#For Canada
Canada_virus = virus %>% filter(Country_Region == "Canada") %>% group_by(Date) %>% summarize(Confirmed = sum(Confirmed),Deaths = sum(Deaths), Recovered = sum(Recovered)) %>% mutate("Province_State" = "Canada", "Country_Region" = "Canada")
...
```{r}
ourfinaldata %>% filter(Date == "2020-03-19")
```
...
ourfinaldata %>% filter(Country_Region == "Korea, South") %>% group_by(Date) %>%  ggplot() + geom_line(aes(x = Date, y = Confirmed,group =1), linetype = "dashed")  + geom_line(aes(x = Date, y = Deaths,group =1), color = "red")+ ggtitle("South Korea Cases and Deaths")
...
ourfinaldata %>% filter(Country_Region == "United Kingdom"&Province_State == "United Kingdom") %>% group_by(Date) %>%  ggplot() + geom_line(aes(x = Date, y = Confirmed,group =1), linetype = "dashed")  + geom_line(aes(x = Date, y = Deaths,group =1), color = "red")+ ggtitle("United Kingdom Cases and Deaths")

#Deaths in France
ourfinaldata %>% filter(Country_Region == "France" & Province_State ==  "France") %>% group_by(Date) %>%  ggplot() + geom_line(aes(x = Date, y = Confirmed,group =1), linetype = "dashed")  + geom_line(aes(x = Date, y = Deaths,group =1), color = "red")+ ggtitle("France Cases and Deaths")
...
virus %>% filter(Country_Region == "United States"& Date == "2020-03-19") %>% top_n(Confirmed, n = 15)%>% arrange(desc(Confirmed))

#Bottom 10 States based on total Cases
virus %>% filter(Country_Region == "United States"& Date == "2020-03-19") %>% top_n(Confirmed, n = -10)%>% arrange(desc(Confirmed))
```
