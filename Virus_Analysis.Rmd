...
num_days = nrow(ourfinaldata %>% filter(Country_Region == "Italy",Confirmed >3))
days = c(1:num_days-1)
logistic_dat = ourfinaldata %>% filter(Country_Region == "Italy",Confirmed >3) %>% mutate("Num_Days_Since_Start"=c(1:num_days-1))
#logistic <- nls(Confirmed~SSlogis(Num_Days_Since_Start),data = logistic_dat)

y <- function(x){
  return(c(120395.1478/(1+367.173043*exp(-0.195399*x))))
}
xlist = c(1:num_days+1)
ylist = list()

for (thing in logistic_dat$Num_Days_Since_Start){ylist[thing] <- y(thing)}
length(ylist)
length(logistic_dat$Num_Days_Since_Start)
#logistic_dat %>% filter(Country_Region == "Italy") %>% group_by(Date) %>%  ggplot() + geom_line(aes(x = Num_Days_Since_Start, y = Confirmed,group =1), linetype = "dashed") +geom_point(aes(x = Num_Days_Since_Start, y = ylist))
#logistic_dat$Num_Days_Since_Start
#ggplot() + geom_point(aes(x = xlist, y = ylist))
```
