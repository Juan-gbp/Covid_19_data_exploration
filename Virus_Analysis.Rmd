...
my_counties %>% filter(Province_State == "Pennsylvania" & Date == "2020-06-03")
...
my_counties %>% filter(Date == "2020-06-03") %>% group_by(Province_State) %>% summarize(state_total = sum(Confirmed)) %>% top_n(state_total,n=20) %>% arrange(desc(state_total))
my_counties %>% filter(Date == "2020-06-03") %>% group_by(Province_State) %>% summarize(state_total = sum(Confirmed)) %>% top_n(state_total,n=-20) %>% arrange(desc(state_total))
...
my_counties %>% filter(Province_State == "Pennsylvania") %>% arrange(desc(Date))
```
