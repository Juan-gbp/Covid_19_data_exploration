...
USA_dat = ourfinaldata %>% filter(Country_Region == "United States",Confirmed >13) %>% mutate("Num_Days_Since_Start"=c(1:num_days_USA-1)) %>% arrange(Date)

y_Italy <- function(x){
  return(c(120395.1478/(1+367.173043*exp(-0.195399*x))))
}
Days = c(1:num_days_Italy)
Days = matrix(xlist,byrow = T,ncol = length(xlist))

Total_Confirmed_Italy = apply(Days,1,y_Italy)
Total_Confirmed_Italy = t(y_vals)
